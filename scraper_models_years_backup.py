...
class ModelYearScraper:
    ...
    def load_system_mappings(self):
        """Load system mappings from make_specs.txt file."""
        try:
            system_mappings = {}
            current_make = None
            
            with open("make_specs.txt", "r", encoding="utf-8") as f:
                lines = f.readlines()
            ...
            for line in lines:
                ...
                # Check if this is a make line
                if not line.startswith('\t'):
                    # New make group (separated by OR)
                    makes = [make.strip() for make in line.split('OR')]
                    current_make = makes
                    for make in makes:
                        system_mappings[make] = {}
                # This is a system line
                elif line.startswith('\t') and current_make:
                    # Parse system type and values
                    parts = line.split(':', 1)
                    if len(parts) == 2:
                        system_type = parts[0].strip()
                        system_values = [val.strip() for val in parts[1].strip().split(',')]
                        
                        # Add to each make in the current group
                        for make in current_make:
                            system_mappings[make][system_type] = system_values
            
            logger.info(f"Loaded system mappings for {len(system_mappings)} make groups")
    ...
    async def process_adas_systems(self, page, manufacturer, model, year_or_chassis):
        ...
        make_key = manufacturer
        for key in self.system_mappings:
            if key.upper() == manufacturer:
                make_key = key
                break
        
        # Check if we have system mappings for this make
        if make_key not in self.system_mappings:
            logger.warning(f"No system mappings found for {manufacturer}")
            return adas_results
            
        mappings = self.system_mappings[make_key]
        
        # Process each ADAS system type
        for system_type, system_options in mappings.items():
            if not system_options or system_options[0] == "N/A":
                logger.info(f"Skipping {system_type} for {manufacturer} - marked as N/A")
                continue
                
            logger.info(f"Processing {system_type} with options: {system_options}")
            
            # Try each system option until one works
            for system_option in system_options:
                try:
                    # Click the System dropdown
                    system_selector = "input[placeholder='System']"
                    system_element = await page.wait_for_selector(system_selector, timeout=3000)
                    ...
                    option_exists = await page.evaluate("""(option) => {
                        const elements = Array.from(document.querySelectorAll('li'));
                    ...
                    if not option_exists:
                        logger.info(f"System option '{system_option}' not found in dropdown")
                        continue
                        
                    # Select the system option
                    result = await self.select_system(page, system_option)
                    
                    if result and result != "N/A":
                        logger.info(f"Found result for {system_type}: {result}")
                        adas_results[system_type] = result
                        break
                    else:
                        logger.info(f"No valid result for system option '{system_option}'")
                except Exception as e:
    ...
    async def select_system(self, page, system_name):
        ...
        try:
            ...
            # Click the System dropdown to open it
            if await self.interact_with_dropdown(page, "System", system_name, True):
                logger.info(f"Successfully selected system: {system_name}")
                
                # Wait for results to load
                await page.wait_for_timeout(2000)
                
                # Look for calibration type indicators
                calibration_type = await self.get_calibration_type(page)
                
                if calibration_type:
                    logger.info(f"Detected calibration type: {calibration_type}")
                    
                    # If static, try to get CSC model
                    ...
                    logger.warning(f"No calibration type found for system: {system_name}")
                    return "N/A"
            else:
                logger.error(f"Failed to select system: {system_name}")
                return "N/A"
    ...
    async def get_csc_code(self, page):
        """Extract CSC code from the page content."""
        try:
            logger.info("Looking for CSC code in page...")
            
            # Look for CSC model patterns in page text
            page_text = await page.evaluate("""() => {
                return document.body.innerText;
            }""")
            
            # Look for patterns like AUTEL-CSC0800, CSC0802/01, etc.
            csc_pattern = r'(?:AUTEL-)?CSC0[80][0-9]{2}(?:/\d+)?'
            matches = re.findall(csc_pattern, page_text)
            
            if matches:
                csc_model = matches[0]
                logger.info(f"Found CSC code in page text: {csc_model}")
                return csc_model
            
            logger.warning("No CSC code found in page text")
            return None
